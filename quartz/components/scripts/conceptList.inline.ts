function setupConceptList() {
  ...
  function getFilters() {
    const category =
      (container.querySelector('[data-filter="category"]') as HTMLSelectElement)?.value ?? "all"
    const confidence =
      (container.querySelector('[data-filter="confidence"]') as HTMLSelectElement)?.value ?? "all"
    const sort =
