...
const SiteNav: QuartzComponent = ({ fileData }: QuartzComponentProps) => {
  ...
  const activeClass = (section: string) => {
    ...
    if (section === "reports" && slug.startsWith("reports/")) return " active"
    if (section === "start" && slug === "start") return " active"
    return ""
  }
  ...
  return (
    <div class="site-nav-container">
      <nav class="site-nav">
        ...
        <div class="nav-links">
          <a
            href={resolveRelative(fileData.slug!, "start" as any)}
            class={`nav-link internal${activeClass("start")}`}
          >
            從這裡開始
          </a>
          <a
          ...
          <a href={resolveRelative(fileData.slug!, "reports/" as any)} class={`nav-link internal${activeClass("reports")}`}>報告</a>
        </div>
        <div class="search">
