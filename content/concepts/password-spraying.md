...
- **log 特徵**：同一時間窗內大量不同帳號各失敗一兩次、且用的都是同一組密碼；不像暴力破解集中在單一帳號。〔CISSP/SSCP 延伸・iPAS 免〕實務上真實身分驗證日誌通常不會記明文密碼，真正的偵測訊號是「同來源／同時間窗、對大量帳號各只少量失敗、刻意壓在帳號鎖定門檻下」的橫向模式，而非在 log 裡看到同一組密碼。
