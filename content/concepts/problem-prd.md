...
- **四格框架變體（現況／目標／障礙／問題規模）**：《AI 超級大腦》CH1 用的「問對問題」是四格版，比三層多了「問題規模」（值不值得解、解多大）一格；三案例（選人／信任門檻／貼地）都靠它凸顯「我以為要解的問題」和「真正要解的問題」之間的落差——落差常比想的大，先對齊落差再往下才不會解錯題。適合當 skill 的前置欄位。
