...
- **長時程生成要驗過程與成品**：程式碼行數、執行時間與 token 成本只描述投入；量表還要檢查模型能否持續維持結構、透過 [[agent-perceptual-feedback-loop]] 看見最終成品，並根據觀察可靠修正。
- **Skill 評估要拆路由與產出**：第一層量「該觸發與不該觸發的要求是否分對」，第二層才量「叫用後輸出是否符合斷言」。每個案例要在全新對話中各跑一次啟用與停用 Skill，比較通過率、時間與 token；若沿用撰寫 Skill 的對話，殘留脈絡會替缺漏的指令補答案，讓結果看起來比實際更好。
