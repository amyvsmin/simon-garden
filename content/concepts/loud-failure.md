...
<p align="center"><img src="assets/covers/loud-failure-cover.png" alt="封面圖" width="400"></p>
...
- **檢查的對象要含「對應關係」**：三支檢查全綠、東西其實壞了，因為腳本檢查檔案內容、壞的是設定與檔案的對應（設定指向 A、檔案在 B、索引靜默不載入）；驗證要涵蓋「誰指向誰」不只「內容對不對」。（蛋糕三棲記憶系統）
- **假陽性太多等於沒有檢查**：外洩掃描把「2,610 個檔案」當金額、斷鏈檢查把說明文件的範例當壞連結，真訊號被淹掉；逐條縮規則到命中數變小（37→1、36→21、21→18）才看得見。（蛋糕三棲記憶系統）
