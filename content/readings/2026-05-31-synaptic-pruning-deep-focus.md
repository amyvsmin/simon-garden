...
- ✅ **course-notes 加「🧠 自我檢測（主動回憶）」區塊**：改 `~/.claude/skills/course-notes/SKILL.md` 的 Body Template + 重點整理寫作指南，lecture 類課程筆記產出時附 2-3 題問答（答案放 `> [!success]-` 折疊），複習時先自答再展開。出處：本篇突觸減除／主動回憶。效益：讓 Google Cyber／CCNA／CISSP 課程筆記支持 active recall、對抗突觸修剪、直接服務證照長線。
- 備註：ccna-quiz 本身已是主動回憶機制（出題→答→對照、錯題 streak 間隔重練），不需另改。

**B. Simon 個人動作**（狀態由 Simon 自己維護）

- ⬜ 每天閱讀換成高認知負荷材料（資安原文／CISSP 教材）— 待試
- ⬜ 重要學習筆記／決策改用手寫 — 待試
- ⬜ 想滑手機時站起來看 6 米外物體三秒 — 待試
