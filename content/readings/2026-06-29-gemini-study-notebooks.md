...
**A 類芙莉蓮優化**（待 Simon 討論，不自己套）：
- 候選：cert-quiz skill 模式一（刷題）開頭加一個可選的「基準診斷起手式」——先刷一輪定弱點、再優先出弱項題，把這篇招 1+3 的「先測後讀、依弱點排序」明確化。改動位置：cert-quiz SKILL.md。**待 Simon 拍板**：他現有的錯題追蹤＋間隔重練可能已涵蓋這個精神，加起手式未必值得多一層複雜度。
