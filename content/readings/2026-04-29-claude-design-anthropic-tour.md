...
- ❌ **ISO 27001 文件視覺化**：資產盤點與風險評估的流程圖、政策架構圖，可用 Hi-Fi 模式快速產草稿給跨部門看，比 Visio 起步快 — 流程圖會用 Excalidraw / Visio 起步快
- ❌ **Knowledge Wiki 概念頁設計**：Claude Design 的「設計系統」概念可借鑑到 Notion 概念頁固定結構，思考是否要為 Knowledge Wiki 訂一套視覺規範（顏色、emoji、callout 用法） — KW 走簡潔風、視覺規範會過度設計
- ❌ **n8n 串接思路**：Netlify Connector 自動部署的概念，跟 n8n 把多個服務串成工作流的思維同源，可加深 Simon 對「服務間自動化」的理解 — n8n 服務間自動化思維已具備
