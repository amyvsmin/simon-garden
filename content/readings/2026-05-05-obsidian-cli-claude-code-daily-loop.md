...
1. ❌ **Obsidian CLI plugin**：跟現有 Obsidian app 整合進來、之後 Claude Code 不必透過檔案系統 read／write，可直接喚 Obsidian 的指令（搜尋、開啟特定 note、操作 callout）。 — 已選 Local REST API plugin 路線
2. ❌ **SessionHandoff.md + 開工收工 loop**：跟 Simon 現有 journal skill（純接收 + 即時落地）互補。journal 是日記語境的純記錄，SessionHandoff 是任務／工作語境的跨 session 接力。可放在 vault `9-ops/SessionHandoff.md` 或 `0-context/`，補上「每日工作交接」這層。 — PreCompact Snapshot + morning skill 已替代
